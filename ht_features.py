...
def calc_num_stats(df_series,window_size,q_values=np.arange(5,100,20)):
    ...
    rolling_mean_feats = calc_rolling_function(df_series,10,col,np.mean)
    rolling_std_feats = calc_rolling_function(df_series,10,col,np.std)
    rolling_min_feats = calc_rolling_function(df_series,10,col,np.min)
    rolling_max_feats = calc_rolling_function(df_series,10,col,np.max)
    rolling_kurtosis_feats = calc_rolling_function(df_series,10,col,kurtosis)
    rolling_skew_feats = calc_rolling_function(df_series,10,col,skew)
...
#    feature_windows_dict = HT_FEATURE_WINDOWS
    ...
    for col in df_grp.columns:
        ...
        if feature_types_dict[col] == 'c':
            feats = calc_cat_stats(df_grp[col],feature_windows_dict[col])
        elif feature_types_dict[col] == 'n':
            feats = calc_num_stats(df_grp[col],feature_windows_dict[col])
        elif feature_types_dict[col] == 'd':
            feats = calc_date_stats(df_grp[col],feature_windows_dict[col])
        elif feature_types_dict[col] == 'cn':
            cat_feats = calc_cat_stats(df_grp[col],feature_windows_dict[col])
            num_feats = calc_num_stats(df_grp[col],feature_windows_dict[col])
...
def gen_grouped_features(df,groupby_key,sort_key,feature_types_dict,feature_windows_dict):
